...
onMounted(async () => {
    state.stations = await fetch('/api/stations').then(res => res.json());
...
<template>
                    ...
                    <div v-if="audio" class="controls__media">
                        <NextIcon class="previous-song" @click="() => {
                            if (audio.currentTime < 5) {
                                ...
                                audio.src = `/api/play?station=${state.currentStation.name}&song=${state.currentSong}`;
                                audio.play();
                                return;
                            }
    ...
    <Modal class="select-song" :open="state.changingSong" @close="() => state.changingSong = false">
        ...
        <template v-slot:body>
            <div
                class="song"
                :class="{'song--disabled': song === state.currentSong}"
                ...
                @click="() => {
                    ...
                    audio.src = `/api/play?station=${state.currentStation.name}&song=${song}`;
                    audio.play();
